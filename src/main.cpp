...
int main(int argc, char* argv[])
{
		...
		if ((arg == "-h") || (arg == "--help"))
			...
			if (i + 1 < argc)
				...
				try
					...
					if (min_support < 0) {
						std::cerr << "-c option requires a numeric argument not lower than 0" << std::endl;
						return 1;
					}
					}
				catch (std::exception&)
	...
	if (!new_measures && min_support > 1) {
		std::cerr << "Min. support with old measures cannot be greater than 1." << std::endl;
	}
